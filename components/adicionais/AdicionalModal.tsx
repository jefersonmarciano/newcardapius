...
interface AdicionalModalProps {
  ...
  onSave: (adicional: any) => void;
}

export default function AdicionalModal({ open, onOpenChange, onSave }: AdicionalModalProps) {
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-5xl rounded-2xl p-8">
        <DialogHeader>
          <DialogTitle className="text-2xl font-bold">Adicional</DialogTitle>
          <DialogDescription>Cadastre as informações do adicional</DialogDescription>
        </DialogHeader>
        <form className="grid grid-cols-2 gap-8 mt-4 text-xs" onSubmit={handleSubmit}>
          <div className="flex flex-col gap-4">
            ...
            <div className="grid grid-cols-4 gap-4">
              <div>
                ...
                <Input type="number" placeholder="R$ 100" value={valor} onChange={e => setValor(e.target.value.replace(/[^0-9]/g, ""))} required />
              </div>
              <div>
          ...
          <hr className="mb-2 mt-8 border-t border-gray-200" />
